...
pub fn parse_block(
    ...
    let account_blocks: Vec<(_, _)> = account_blocks
        .iter()
        .filter_map(|account_block| match account_block {
            Ok((builder, mut slice_data)) => {
                ...
                match AccountBlock::construct_from(&mut slice_data) {
                    Ok(a) => Some((address, a)),
                    Err(e) => {
                        dbg!(e);
                        None
...
mod test {
    ...
    fn parse() {
        ...
        let res = parse_block(&fns, &block).unwrap();
        dbg!(res);
    }
}
